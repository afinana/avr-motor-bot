...
// FIXME: pdf-ize that last datasheet ref
#define ADC_DISABLE()                       \
  do {                                      \
    ADCSRA &= ~(_BV (ADEN));                \
...
// Re-enable ADC hardware.  The first conversion performed (with
// adc_read_raw() or adc_read_voltage()) after this takes 25 ADC clock cycles,
// rather than the usual 13 ADC clock cycles.  See FIXME: pdfize this ref:
// ATMega328P datasheet section 23.4 for details.
#define ADC_ENABLE()                      \
  do {                                    \
    ADCSRA |= _BV (ADEN);                 \
