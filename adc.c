...
adc_init (adc_reference_source_t reference_source)
{
  ...
  // (ADC needs a 50 - 200kHz clock), and start a sample.  FIXME: pdf-ize
  // this reference: The ATmega328P datasheet specifies that the first sample
  // taken after the voltage reference is changed should be discarded.
...
adc_read_voltage (uint8_t pin, float reference_voltage)
{
    ...
    return ((float) raw / (ADC_RAW_READING_STEPS - 1)) * reference_voltage;
}
