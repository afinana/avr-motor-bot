...
# See the desciprtion of the SCREEN_SESSION_NAME variable below.
#USE_SCREEN_SESSION_PER_DIRECTORY = defined
...
# If USE_SCREEN_SESSION_PER_DIRECTORY is defined and not empty, we embed the
# directory from which the recipes that evaluates this variable are run to
# support the use case where multiple Arduinos are connected simultaneously:
# they're hopefully being edit-compile-debugged from different directories,
# so we'll get a unique screen session name for each.  Otherwise we use
# a single session with a fixed name.  This supports the probably more
# common use case where there's only one Arduino connected and we're
# reprogramming it from different directories (to perform differential
# testing for example).  Note that this variable is expanded when used, so
# USE_SCREEN_SESSION_PER_DIRECTORY can be defined after this file is included.
SCREEN_SESSION_NAME =                                            \
  $(strip $(if $(USE_SCREEN_SESSION_PER_DIRECTORY),              \
               cduino_run_screen_from_$(subst /,_,$(shell pwd)), \
               cduino_run_screen_mk ) )
